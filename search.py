...
def graph_search(problem, fringe, bab=None, babh=None):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            return node, open, close, node.path_cost
        if node.state not in closed:
            ...
            if babh:
                fringe.sort(key=lambda x: x.path_cost + problem.h(x), reverse=True)
    return None
...
def bab(problem):
    return graph_search(problem, [], True)

def babh(problem):
    return graph_search(problem, [], babh=True)
